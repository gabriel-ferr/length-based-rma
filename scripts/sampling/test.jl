...
# --------------------------------------------------------------------------------
results = Array{Float64, 7}(undef, NUM_INITIAL_CONDITIONS, length(THRESHOLD_RANGE_SAMPLING), length(SAMPLING_RATIO_RANGE), 5, 3, 2, 2)
# --------------------------------------------------------------------------------
@showprogress for i ∈ 1:NUM_INITIAL_CONDITIONS
    ...
    for j ∈ eachindex(THRESHOLD_RANGE_SAMPLING)
        for s ∈ eachindex(input)
            rp = RecurrenceMatrix(input[s], dmax[s] * THRESHOLD_RANGE_SAMPLING[j])
            # -----
            results[i, j, 1, s, 1, 1, 1] = dl_average(rp; lmin = 1)
            results[i, j, 1, s, 1, 1, 2] = dl_average(rp; lmin = 2)

            results[i, j, 1, s, 2, 1, 1] = trappingtime(rp; lmin = 1)
            results[i, j, 1, s, 2, 1, 2] = trappingtime(rp; lmin = 2)

            results[i, j, 1, s, 3, 1, 1] = meanrecurrencetime(rp; lmin = 1)
            results[i, j, 1, s, 3, 1, 2] = meanrecurrencetime(rp; lmin = 2)
            # -----
            for n ∈ eachindex(SAMPLING_RATIO_RANGE)
                results[i, j, n, s, 1, 2, 1] = complexity(RecurrenceAverageDiagonal(1, dmax[s] * THRESHOLD_RANGE_SAMPLING[j]; ratio = SAMPLING_RATIO_RANGE[n]), input[s])
                results[i, j, n, s, 1, 2, 2] = complexity(RecurrenceAverageDiagonal(2, dmax[s] * THRESHOLD_RANGE_SAMPLING[j]; ratio = SAMPLING_RATIO_RANGE[n]), input[s])

                results[i, j, n, s, 2, 2, 1] = complexity(RecurrenceTrappingTime(1, dmax[s] * THRESHOLD_RANGE_SAMPLING[j]; ratio = SAMPLING_RATIO_RANGE[n]), input[s])
                results[i, j, n, s, 2, 2, 2] = complexity(RecurrenceTrappingTime(2, dmax[s] * THRESHOLD_RANGE_SAMPLING[j]; ratio = SAMPLING_RATIO_RANGE[n]), input[s])

                results[i, j, n, s, 3, 2, 1] = complexity(RecurrenceMeanTime(1, dmax[s] * THRESHOLD_RANGE_SAMPLING[j]; ratio = SAMPLING_RATIO_RANGE[n]), input[s])
                results[i, j, n, s, 3, 2, 2] = complexity(RecurrenceMeanTime(2, dmax[s] * THRESHOLD_RANGE_SAMPLING[j]; ratio = SAMPLING_RATIO_RANGE[n]), input[s])
            end
            # -----
        end
    end
...
# --------------------------------------------------------------------------------
BSON.@save datadir("results_sampling.bson") results